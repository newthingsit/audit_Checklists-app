...
async function runMigrations(dbInstance) {
  try {
    // Migration: Update "SERVICE - Speed of Service" category in "CVR - CDR" template
    // This runs once and is idempotent (checks if already done)
    const MIGRATION_KEY = 'speed_of_service_update_v1';
    
    // Check if migration already ran
    const checkMigration = await dbInstance.get(
      "SELECT id FROM app_settings WHERE setting_key = ?",
      [MIGRATION_KEY]
    );
    
    if (checkMigration) {
      logger.info('[Migration] Speed of Service already updated, skipping');
      return;
    }
    
    logger.info('[Migration] Running Speed of Service update...');
    
    // Find the template
    const template = await dbInstance.get(
      "SELECT id FROM checklist_templates WHERE name = ?",
      ['CVR - CDR']
    );
    
    if (!template) {
      logger.info('[Migration] CVR - CDR template not found, skipping migration');
      return;
    }
    
    const templateId = template.id;
    const category = 'SERVICE - Speed of Service';
    ...
    const ITEMS = [
      ...
      { title: 'Was the complete food order served in a timely manner?', yesMark: '0' },
    ...
    logger.info(`[Migration] Deleted ${existing.length} existing items`);
    ...
    await dbInstance.run(
      ...
      [MIGRATION_KEY, new Date().toISOString(), 'Speed of Service category updated with curated items']
    );
    
    logger.info(`[Migration] Successfully inserted ${insertedCount} items in Speed of Service category`);
    logger.info('[Migration] Speed of Service update completed!');
    
  } catch (err) {
    logger.error('[Migration] Error during Speed of Service update:', err.message);
